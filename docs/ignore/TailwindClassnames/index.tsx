...
const TailwindClassnames = () => {
  ...
  Object.entries(baseConfig).forEach(([key, value]) => {
    if (key === "colors") {
      transformed[upperFirst(key)] = transformColors(value as Record<string, string>);
    } else {
      transformed[upperFirst(key)] = value;
    }
  });

  const { Keyframes: _Keyframes, Extend: _Extend, ...transformedConfig } = transformed;
