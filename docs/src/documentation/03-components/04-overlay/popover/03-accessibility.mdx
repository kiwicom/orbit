...
The Popover's trigger element (provided as `children`) should not be an interactive element like a native `<button>` or `<a>` tag, as this creates nested interactive elements which is an accessibility violation. Instead:

- If using a Button component as a trigger, set `asComponent="div"` to avoid this issue:
...
```jsx
<Popover content="This is the popover content" ariaLabel="Example popover">
  <Button asComponent="div">Open popover</Button>
</Popover>
```

#### Advanced accessible popover
...
<div>
  ...
  <Popover
    ...
    labelClose="Close passenger selection"
  >
...
In this example, screen readers would announce "Passenger selection" when the popover opens, based on the associated heading. The close button would be announced as "Close passenger selection".
