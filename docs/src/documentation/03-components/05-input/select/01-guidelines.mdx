...
- When users can select more than one of a few optioons---use a [checkbox](/components/input/checkbox/).
...
<FigmaFile
  ...
  node="171%3A1252"
  alt="Label: provides a clear description of what users should fill in; placeholder: provides additional help for expected values; prefix: optionally adds additional clarity for options; select suffix: indicates there are options to choose from."
/>
...
When you have only a few options so users won't be overwhelmed by them,
