---
title: EmptyState
description: Indicates when there’s no data to show, like when a search has no results.
redirect_from:
...
Guide users out of the empty state into their happy state.
...
Don't blame users for the empty state.
