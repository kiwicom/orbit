---
title: Guidelines
redirect_from:
  - /components/skeleton/
---

<ReactExample exampleId="Skeleton-default" />

## When to use

- When you need to display placeholder preview of your content before the data is loaded
- When component has large content
- To keep users from giving up on an experience.

## Component status

<ComponentStatus component="Skeleton" />

## Behavior

### Default

By default **Skeleton** component have single row.
You can change number of rows, their height and offset between them. It's useful if you need to make it equal to expected height of the upcoming data.

### Presets

**Skeleton** component have some premade presets, it's useful, when you need to build the layout with different components like `Button`, `Text`, `Image`, `Card` and etc.

<ReactExample exampleId="Skeleton-presets" />

### Animation and color

You can change the look of the component by chaining background and foreground colors. Animation can be turned on/off, the speed and interval of animation is adjustable.

<ReactExample exampleId="Skeleton-animation" />
