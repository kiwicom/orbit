...
![A long page with buttons in 3 different configurations: 1. everything primary, 2. one primary
  and the rest secondary, 3. everything secondary.](fileId:uK3xATJqPZwNwV5KT7igBG;nodeId:3%3A5 "On
  the left, we can see that when everything is important, nothing is. To fix this, either highlight
  the one most important action (the middle option) or make them all less distinct (the option on
  the right).")

To help users through this, you want to **simplify** your designs to highlight what's important. It's
important to remember to make it clear that the secondary choices are still available and how users can
get to them if they want.
...
![An input field with the text 'Add place'
  and some suggested
  destinations.](../../images/design-patterns/NomadAddDestination.png)
...
![The text 'London' and 'Stay 3--5 nights',
  with the length of stay underlined and an edit button next to it
  .](../../images/design-patterns/NomadHoverLength.png)
...
![The same text as before,
  now with a slider underneath to select the length of stay from 1 to 60 nights
  .](../../images/design-patterns/NomadChangeLength.png)
...
<ReactExample exampleId="Truncate-default" height={200} />
...
<ReactExample exampleId="Collapse-search_filter" height={200} />
...
<ReactExample exampleId="Popover-passengers" height={240} />
...
You can see an example in the list below.
The features are presented briefly to ease comparison.
But each feature has a tooltip for explaining additional details.
This can separate more complex information.

<ReactExample exampleId="Tooltip-progressive_disclosure" />
