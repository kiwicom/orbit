...
A new `navbar` elevation level has been added and box shadow values for all elevation levels (level1 through level4) have been updated. These changes affect components using elevation tokens like cards, modals, navigation bars, dropdowns and tooltips.
