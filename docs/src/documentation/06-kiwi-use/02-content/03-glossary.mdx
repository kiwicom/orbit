...
: Contains travel details and enables check-in but does not allow a passenger to board.
...
Each itinerary will have a unique PNR associated with all the booking and personal details.
...
When you're traveling by bus, it is a **bus segment**. And so on.
...
and then from Madrid to Vienna will have three sectors.
