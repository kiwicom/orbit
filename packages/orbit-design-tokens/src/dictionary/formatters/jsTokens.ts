 ...
 */
const transformObjectReferece = (value: string) => {
...
const createSpacingValue = (values: string[]) => {
  ...
  const value = values.map(v =>
    !/[\d]+px/g.test(v) && String(v) !== "0" ? `\${${transformObjectReferece(v)}}` : v,
  );
  return `\`${value.join(" ")}\``;
};
...
const typescriptFactory = (allProperties: Dictionary["allProperties"], complete = true) => {
  ...
  const tokens = _.map(allProperties, prop => {
    ...
    const val = typeof plainValue === "string" ? transformObjectReferece(plainValue) : plainValue;
