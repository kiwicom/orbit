...
const ModalWrapperContent = styled.div`
  ...
  height: calc(
    100% - ${({ theme }) => theme.orbit.spaceXLarge} -
      ${({ contentIndent }) => (contentIndent ? "76px" : "0px")}
  );
  ...
  ${StyledModalSection}:last-of-type {
    padding-bottom: ${({ theme, contentIndent }) => contentIndent && theme.orbit.spaceLarge};
    margin-bottom: ${({ contentIndent }) => contentIndent && "0"};
  }
...
const MobileHeader = styled.div`
  ...
  top: ${({ scrolled, theme }) => (scrolled ? theme.orbit.spaceXLarge : "-52px")};
  height: 50px;
  width: 100%;
  ...
  transition: top ${({ theme }) => theme.orbit.durationNormal} ease-in-out;
  // TODO: use token boxShadowActionable
...
const getIndent = children =>
  React.Children.map(
    children,
    item => (item.type === ModalFooter ? item.props.fixed : false),
  ).includes(true);

const getTitle = children =>
  React.Children.map(children, item => (item.type === ModalHeader ? item.props.title : undefined));

const Content = ({ closable, onClose, size, children, scrolled }: ContentType) =>
  closable && onClose ? (
    <ClickOutside onClickOutside={onClose}>
      <ModalWrapperContent size={size} contentIndent={getIndent(children)} scrolled={scrolled}>
    ...
    <ModalWrapperContent size={size} contentIndent={getIndent(children)} scrolled={scrolled}>
      <MobileHeader scrolled={scrolled}>{getTitle(children)}</MobileHeader>
      {children}
    </ModalWrapperContent>
  );
...
class Modal extends React.PureComponent<Props, State> {
  ...
  render() {
    const { onClose, children, size = SIZES.NORMAL, closable = true } = this.props;
    ...
    return (
      <ModalBody tabIndex="0" onKeyDown={closable ? this.handleKeyDown : undefined}>
        ...
        >
          <Content closable={closable} onClose={onClose} size={size} scrolled={scrolled}>
            {children}
          </Content>
        </ModalWrapper>
      </ModalBody>
